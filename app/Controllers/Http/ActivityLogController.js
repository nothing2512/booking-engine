...
class ActivityLogController {
    ...
    async show({response, request, params}) {
        ...
        if (user instanceof User) {
            ...
            if (user.role_id == 3) user[`${Engine.lower("aggregator")}Profile`] = await user.aggregatorProfile().fetch();
            if (user.role_id == 2) {
                let subquery = Database.from('aggregator_readers')
                    .where('reader_id', user.id)
                    .select('aggregator_id');
                user[`${Engine.lower("aggregator")}Profile`] = await AggregatorProfile.findBy('user_id', subquery)
            }
