...
class StatisticController {
    ...
    async index({auth, response}) {
        ...
        if (user instanceof User) {
            let reader_ids = Database.from('aggregator_readers').where('aggregator_id', user.id).select('reader_id')

            statistics.reader = {
                active: await User.query().where('role_id', 2).whereIn('id', reader_ids).where('status', 'active').getCount(),
                inactive: await User.query().where('role_id', 2).whereIn('id', reader_ids).where('status', 'inactive').getCount(),
                banned: await User.query().where('role_id', 2).whereIn('id', reader_ids).where('status', 'banned').getCount(),
                total: await User.query().where('role_id', 2).whereIn('id', reader_ids).getCount()
            }

            statistics.consultation = {
                need_paid: await Consultation.query().whereIn('reader_id', reader_ids).where('status', 0).getCount(),
                paid: await Consultation.query().whereIn('reader_id', reader_ids).where('status', 1).getCount(),
                done: await Consultation.query().whereIn('reader_id', reader_ids).where('status', 2).getCount(),
                expired: await Consultation.query().whereIn('reader_id', reader_ids).where('status', 3).getCount(),
                rejected: await Consultation.query().whereIn('reader_id', reader_ids).where('approval_status', 1).getCount(),
                approved: await Consultation.query().whereIn('reader_id', reader_ids).where('approval_status', 2).getCount(),
                total: await Consultation.query().whereIn('reader_id', reader_ids).getCount()
            }
